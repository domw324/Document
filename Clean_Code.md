...
##### 단순 설계 규칙 2~4 : 리팩토링
코드를 점진적으로 리팩토링 한다. 이때 이미 테스트 케이스가 잘 짜져있다면 코드를 정리하면서 시스템이 깨지지 않는다. (깨지려고 하면 알려주니까!)
...
## 13. 동시성
동시성을 만족하면서 깔끔한 코드를 짜는 것은 매우 어렵다. 그리고 여러 스레드를 동시에 돌리는 것 또한 어렵다. 어떻게 하면 이런 어려움을 극복하고 개끗한 코드를 작성할 수 있을까. 그리고 동시성은 어떻게 테스트할 수 있을까.
##### 동시성 - 필요성
동시성은 **무엇**<sup>what</sup>과 **언제**<sup>when</sup>를 분리하는 전략이다. 즉, **결합성**<sup>coupling</sup>을 제거할 수 있다. 동시성이 좋아지면 애플리케이션 구조와 효율이 극적으로 좋아진다. 하지만, 동시성 구현은 절대 쉽지 않다.
- 오해하기 쉬운 점
  - **동시성이 항상 성능을 높여주지 않는다.** 대기시간이 아주 긴 여러 스레드가 프로세스를 공유할 수 있거나, 여러 프로세서가 동시에 처리할 독립적 계산이 충분히 많은 경우에만 성능이 높아진다.
  - 동시성을 구현하는 경우 일반적으로 **시스템 구조**가 크게 달라진다.
  - 웹이나 EJB 컨테이너를 사용하더라도 동시성을 이해할 필요가 있다. 실제 컨테이너 동작 방법, 동시 수정 방법, 데드락 회피와 같은 지식을 꼭 알아야만 한다.
- 주의할 점
  - 동시성은 다소 **부하**를 유발한다.
  - 동시성은 **복잡**하다.
  - 일반적으로 **동시성 버그**는 재현하기 어렵다.
  - 동시성을 구현하려면 **근본적인 설계 전략**을 재고해야 한다.
##### 동시성 - 방어 원칙
- 단일 책임 원칙<sup>Single Responsibility Principle, SRP</sup>
  - 주어진 메서드, 클래스, 컴포넌트를 변경할 이유가 하나여야 한다는 원칙
  - 동시성 관련 코드는 다른 코드와 분리해야 한다.
- 따름 정리<sup>corollary</sup> : 자료 범위 제한
  - 서로 다른 스레드들이 자료를 공유하되, 간섭하지는 못하게 하자.
  - 공유 객체를 사용하는 코드 내 임계영역<sup>critical section</sup>을 **synchronized** 키워드로 보호
  - 자료 **캡슐화**
  - **공유 자료 최소화**
- 따름 정리 : 자료 사본 사용
  - 공유 자료를 줄이기 위해 처음부터 공유하지 않는 방법이 가장 좋다.
  - 객체를 복사해 사용
- 따름 정리 : 가능한 스레드는 독립적으로 구현
  - 가능한 각 스레드는 클라이언트 요청 하나를 처리하며, 모든 정보는 비공유 출처에서 가져와 로컬 변수에 저장하도록 한다.
  - 자료를 독립적인 단위로 분할 (로컬 변수 사용)
##### 라이브러리를 이해할 것
- 언어가 제공하는 클래스를 검토
...
- 공유 객체 하나에는 메서드를 하나만 사용해야 한다.
  - 메서드 사이에 의존성이 존재하면 동시성 코드에 찾아내기 어려운 버그가 생긴다.
- 공유 객체 하나에 여러 메서드가 필요한 상황에서 고려할 방법
  - 클라이언트에서 잠금 - 클라이언트에서 첫 번 째 메서드를 호출하기 전에 서버를 잠근다. 마지막 메서드를 호출할 때까지 잠금 유지.
  - 서버에서 잠금 - 서버에다 '서버를 잠그고 모든 메서드를 호출한 후 잠근을 해제하는 메서드'를 구현, 클라이언트는 이 메서드를 호출
  - 연결<sup>Adapted</sup> 서버 - 잠근을 수행하는 중간 단계를 생성. (서버에서 잠근 방식과 유사하지만 원래 서버는 변경하지 않음.)
##### 동기화하는 부분을 작게 제작
- 코드를 짤 때는 임계영역 수를 최대한 줄인다. 즉, 동기화하는 부분을 최대한 작게 만든다.
- 필요이상으로 임계영역 크기가 커지면 스레드 간에 경쟁이 늘어나고 프로그램 성능이 떨어진다.
##### 올바른 종료 코드 구현
- 종료 코드를 개발 초기부터 고민하고 동작하게 초기부터 구현. 이미 나온 알고리즘을 검토 할 것
  - 올바른 종료 코드가 불려야 데드락 현상을 피할 수 있다.
##### 스레드 코드 테스트
- 문제를 노출하는 테스트 케이스를 작성.
  - 프로그램 설정과 시스템 설정과  부하를 바꿔가며 자주 돌릴 것
  - 테스트 실패 시 꼭 원인을 추적할 것.
- 말이 안되는 실패는 잠정적인 스레드 문제로 취급
  - 시스템 실패를 일회성이라 치부하지 말 것
- 다중 스레드를 고려하지 않은 순차 코드부터 제대로 동작하도록 제작
  - 스레드 환경 밖에서 생기는 버그와 스레드 환경에서 생기는 버그를 동시에 디버깅 금지
  - 스레드 환경 밖에서 코드를 올바로 돌릴 것
- 다중 스레드를 쓰는 코드 부분을 다양한 환경에 쉽게 끼워 넣을 수 있도록 스레드 코드 구현
  - 한 스레드로 실행 or 여러 스레드로 실행 or 실행 중 스레드 수 변경
  - 스레드 코드를 실제 환경이나 테스트 환경에서 시험
  - 테스트 코드를 빨리, 천천히, 다양한 속도로 시험
  - 반복 테스트를 가능하도록 테스트 케이스 작성
- 다중 스레드를 쓰는 코드 부분을 상황에 맞춰 조정할 수 있게 작성
- 프로세서 수보다 많은 스레드를 돌릴 것
- 다른 플랫폼에서 돌려 볼 것
  - 처음부터, 자주 모든 목표 플랫폼에서 코드를 시험
- 코드에 보조 코드를 넣어 돌릴 것. 즉, 강제로 실패를 일으켜 볼 것
  - 직접 구현
  - 자동화

## 14. 점진적인 개선
글을 쓸 때 먼저 초안부터 작성하고 그 초안을 여러번 고쳐 최종안을 작성한다.  
이와 같이 깨끗한 코드는 한번에 뚝딱 나오지 않는다. 먼저 ~~지저분한~~ 코드를 짠 뒤에 계속해서 정리해나가는 과정이 꼭 필요하다. 끊임없는 리팩토링<sup>Refactoring</sup>이 필요하다. 동시에 테스트 주도 개발<sup>Test-Driven Development, TDD</sup>을 활용하면 기존 시스템을 망가뜨리지 않고 효과적으로 리팩토링을 수행할 수 있다.